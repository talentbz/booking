...
function changeGalleryOrder(id) {
    var val = $("#orderChange_" + id).val();
    console.log(val, id);
    var origin_array_str = $('#gallery_id_change').val();
    var origin_array = origin_array_str.split(',');
    var temp_array = new Array();

    for(var i = 0; i < origin_array.length; i++){
        if((i + 1) == val){
            temp_array.push(id);
            temp_array.push(origin_array[i]);
        }else{
            if(origin_array[i] != id){
                temp_array.push(origin_array[i]);
            }
        }
    }

    $("#gallery_id_change").val(temp_array.toString());
    console.log($("#gallery_id_change").val());
}
