...
(function ($) {
    ...
    var HHOptions = {
        ...
        initCustomBook: function (el) {
            let base = this;
            let parent = $('.field-custom_booked', el);

            let modal = $('#hh-custom-book-edit-modal', el),
                loading = $('.hh-loading', modal);

            $('.add-book', modal).click(function (ev) {
                loading.show();
                base.initValidation(modal, true);
                if (Object.size(base.isValidated)) {
                    let $el = $('.has-validation.is-invalid', modal).first();
                    ...
                    ev.preventDefault();
                    let data = {
                        start_date: $("#start_date_book", modal).val(),
                        end_date: $("#end_date_book", modal).val(),
                        post_id_bulk: $('#custom_book_id', modal).val(),
                        post_type: $('[name="custom_book_type"]', modal).val(),
                        _token: $('meta[name="csrf-token"]').attr('content')
                    };
                    $.post(modal.data('action'), data, function (respon) {
                        if (typeof respon == 'object') {
                            base.alert(respon);
                            if (respon.status === 1) {
                                $('.book-render', parent).html(respon.html);
                                base.initOnOff($('.book-render', parent));
                            }
                        }
                        loading.hide();
                    }, 'json');
            ...
            parent.on('click', '.delete-book', function (ev) {
                ev.preventDefault();
                var t = $(this),
                    container = t.closest('.field-custom_booked');
                $.confirm({
                    animation: 'none',
                    title: t.attr('data-title'),
                    content: t.attr('data-content'),
                    buttons: {
                        ok: {
                            ...
                            action: function () {
                                let data = {
                                    availiabilityID: t.data('availiability-id'),
                                    postID: t.data('post-id'),
                                    postType: t.data('post-type'),
                                    _token: $('meta[name="csrf-token"]').attr('content')
                                };
