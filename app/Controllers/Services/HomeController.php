...
class HomeController extends Controller
{
    ...
    public static function _importIcal()
    {
        $home_model = new Home();
                                    ...
                                    if ($dtstart >= $today) {
                                        ...
                                        if (!$hasAvai) {
                                            $data = [
                                                ...
                                                'end_time' => $dtend,
                                                'end_date' => $dtend,
                                                'start_time_h' => date("Y-m-d H:i:s",$dtstart),
                                                'start_date_h' => date("Y-m-d H:i:s",$dtstart),
                                                'end_time_h' => date("Y-m-d H:i:s",$dtend),
                                                'end_date_h' => date("Y-m-d H:i:s",$dtend),
                                            ];
                                            if ($total_minutes != 1440) {
    ...
    public function _sendEnquiryForm(Request $request)
    {
        ...
        if ($email == $partner->email) {
            return $this->sendJson([
                'status' => 0,
                'message' => __('Can not send inquery')
            ]);
        }

        // $channel = new \Messenger_Channel_Model();
        // $chatController = ChatController::get_inst();
        ...
        //     $chatController->_create_channel_checking($channel_id, $from_user);
        ...
        //     }
        // } else {
        //     $chatController->_create_channel_checking($has_channel->channel_id, $from_user);
        //     $messenger = new \Messenger_Model();

        //     $new_message = $messenger->newMessage([
        //         'channel_id' => $has_channel->channel_id,
        //         'from_user' => $from_user,
        //         'to_user' => $to_user,
        //         'message' => esc_html($message),
        //         'attachment' => '',
        ...
        // send_mail(esc_html($email), esc_html($name), $admin->email, sprintf(__('[%s] Have a booking request from [Home ID: %s] - %s'), get_option('site_name'), $post_id, get_translate($post->post_title)), balanceTags($message));
        send_mail(esc_html($email), esc_html($name), $partner->email, sprintf(__('[%s] Have a booking request from [Home ID: %s] - %s'), get_option('site_name'), $post_id, get_translate($post->post_title)), balanceTags($message));
        
        $InboxEmail = new InboxEmail;
        $InboxEmail->to = $partner->email;
        $InboxEmail->from = esc_html($email);
        $InboxEmail->subject = sprintf(__('[%s] Have a booking request from [Home ID: %s] - %s'), get_option('site_name'), $post_id, get_translate($post->post_title));
        $InboxEmail->body = balanceTags($message);
        $InboxEmail->sent = 1;
        $InboxEmail->author = 0;
        $InboxEmail->save();
    ...
    public function _getHomeAvailabilityCustomBook($home_id){
        $avai_model = new HomeAvailability();
        $data = $avai_model->getHomeAvailabilityCustomBook($home_id);
        return $data;
    }

    public function _addNewCustomBook(){

        $post_type = request()->get('post_type', 'home');
        $responsive = [];
        $responsive = $this->_addNewCustomHomeBook();

        return $this->sendJson($responsive);
    }

    public function _addNewCustomHomeBook() {
        ...
        $availability_model->_saveCustomAvailability($postID, $start_date, $end_date);

        $this->sendJson([
            'status' => 1,
            'title' => __('System Alert'),
            'message' => __('Created successfully'),
            'html' => view('dashboard.components.services.home.custom_availability', ['custom_booked' => $this->_getHomeAvailabilityCustomBook($postID), 'post_id' => $postID])->render(),
        ], true);
    }

    public function _deleteCustomAvailabilityItem(Request $request)
    {
        $availiabilityID = request()->get('availiabilityID');
        $postID = request()->get('postID');
        $postType = request()->get('postType');
        $serviceObject = get_post($postID, 'home');
        $availability_model = new HomeAvailability();
        
        $delete = false;
        if(is_owner()) {
            $delete = $availability_model->updateType($availiabilityID, 'Delete Request');
            $currentUser = get_current_user_data();
            $admin = get_admin_user();
            $partner = get_user_by_id($serviceObject->author);
            $info = $availability_model->where('ID', $availiabilityID)->get();
            $start_date = date('m.d.Y.', $info[0]['start_date']);
            $end_date = date('m.d.Y.', $info[0]['end_date']);
            $title = "Custom Booked Setting Delete Request";
            $subject = "Custom Booked setting Delete Request from ". $currentUser->email;
            $message = "Now ". $currentUser->email ." requested to delete custom Booked date from ".$start_date." to ".$end_date; 
            ...
            $delete = $availability_model->deleteAvailabilityByID($availiabilityID);
        }
        ...
        if ($delete) {
            $this->sendJson([
                ...
                'html' => view("dashboard.components.services.{$postType}.custom_availability", ['custom_booked' => $this->_getHomeAvailabilityCustomBook($postID), 'post_id' => $postID])->render(),
            ], true);
        }
        $this->sendJson([
            'status' => 0,
                ...
                for ($i = $startTime; $i <= $endTime; $i = strtotime('+1 day', $i)) {
                    ...
                    if ($status == 'available') {
                        ...
                        foreach ($total_array as $range) {
                            ...
                            if ($i >= $range->start_time && $i <= $range->end_time) {
                                if($range->first_minute == 'on' || $range->last_minute == 'on') {
                                    $event = (float)$range->price_per_night - ($range->price_per_night * ($range->discount_percent) / 100);
                                }else if($range->price == 0 && $range->price_per_night > 0){
                                    $event = (float)$range->price_per_night;
                                }else {
                                    $event = (float)$range->price_per_night;
    ...
    public function _reviewTip(Request $request, $service_type = 'home', $service_id)
    {
        $serviceObject = get_post($service_id, $service_type);
        if (is_null($serviceObject) || !$serviceObject || $serviceObject->status != 'publish') {
            return view('frontend.404');
        } else {
            return view('frontend.review-tip', ['post' => $serviceObject, 'service_type' => $service_type]);
        }
    }
    ...
    public function _deleteHomeItem(Request $request)
    {
        ...
        $delete = false;
        if(is_owner()) {
            $delete = $home_model->updateStatus($home_id, 'delete_request');    
            $message = "Poštovani,<br>Vaš zahtjev za brisanjem Ville poslan je administratoru na odobrenje.<br>Nakon pregleda ukoliko je sve u redu Villa će biti obrisana.";
            $currentUser = get_current_user_data();
            $homeObj = $home_model->getById($home_id);
            $admin = get_admin_user();
            $agent = get_user_by_id($homeObj->author);
            send_mail($admin->email, 'Luxury Croatia Retreats', $currentUser->email, 'Owner request delete villa', sprintf('Villa [%s] is requested delete by %s', get_translate($homeObj->post_title), $currentUser->email));
            send_mail($agent->email, 'Luxury Croatia Retreats', $currentUser->email, 'Owner request delete villa', sprintf('Villa [%s] is requested delete by %s', get_translate($homeObj->post_title), $currentUser->email));
        }else {
    ...
    public function _updateHome(Request $request)
    {
        ...
        if ($fields) {
            ...
            foreach ($fields as $field) {
                ...
                if(empty($value)) {
                    if ($field['field_type'] == 'meta') {
                        if($field['section'] == 'gallery_options') {
                            $data[$field['id']] = $value;
                        }
                    }
                }
            ...
            if(count($fields) > 0) {
                if($fields[0]['section'] == 'amenities_options') {
            ...
            if (!empty($data)) {
                ...
                // }
                $home = new Home();
                $slug = Str::slug(esc_html(request()->get($post_title_field, '')));
                $homeObj = $home->getById($postID);
                if(empty($slug)) {
                    $slug = str_replace(' ', '-', str_replace(['[', ']'], '_', get_translate($homeObj->post_title)));
                }else {
                    $slug = str_replace(' ', '-', str_replace(['[', ']'], '_', $slug));
                }
                $slug = date('m-Y').'-'.$slug.'-'.get_translate($homeObj->location_city).'-'.get_translate($homeObj->location_country).'-'.$postID;
                $data['post_slug'] = $slug;
            ...
            if(is_owner()) {
                $message = "Poštovani,<br>Vaša Villa nije aktivna dok pregled unosa ne pregleda i ne odobri agent.<br>Isto će se izvršiti u nakraćem mogućem roku.";
            }
            $respon = [
                'status' => 1,
                'title' => __('System Alert'),
            ...
            if ($step == 'finish' && !empty($redirect) && $event != 'tab') {
                ...
                if(is_owner()){
                    ...
                    $admin = get_admin_user();
                    $agent = get_user_by_id($homeObj->author);
                    send_mail($admin->email, 'New Villa Regiaterd', $currentUser->email, 'Owner Registered Villa', sprintf('Villa [%s] is registered by %s', get_translate($homeObj->post_title), $currentUser->email));
                    send_mail($agent->email, 'New Villa Regiaterd', $currentUser->email, 'Owner Registered Villa', sprintf('Villa [%s] is registered by %s', get_translate($homeObj->post_title), $currentUser->email));
                }
            }
    ...
    public function getSpecialOffering($data = []){
        $home = new Home();
        return $home->getSpecialOffering($data);
    }

    public function getAgent() {
        $service = DB::table('home')->where('owner', get_current_user_id())->get()->first();
    ...
    public function _getAvailabilityHome2(Request $request)
    {
        $events['events'] = [];
        $startTime = strtotime(request()->get('start'));
        $endTime = strtotime(request()->get('end'));
            ...
            if ($homeObject->booking_type == 'per_night') {
                ...
                for ($i = $startTime; $i <= $endTime; $i = strtotime('+1 day', $i)) {
                    ...
                    foreach ($avaiItems['results'] as $avaiItem) 
                    {
                        ...
                        else if( $i == strtotime('+1 day', $avaiItem->end_date)) 
                        {
                            $status = 'endday';
                            $event = 'End_day';
                            break;
