...
class ExperienceController extends Controller
{
    ...
    public function _getSearchResult()
    {
        ...
        $locations = [];
        if ($data['total'] > 0) {
            $column = (!isset($post_data['showmap']) || (isset($post_data['showmap']) && $post_data['showmap'] == 'yes')) ? 'col-12 col-sm-12 col-md-12' : 'col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12';
            foreach ($data['results'] as $k => $item) {
                ...
                // $html .= '</div>';
                $html .= view('frontend.experience.loop.list', ['item' => $item])->render();
                $locations[] = [
