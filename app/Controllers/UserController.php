...
class UserController extends Controller
{
    ...
    public function _afterRegisterOwner(Request $request, $return)
    {
        $return = json_decode(base64_decode($return));
        if (is_object($return)) {
            $user_id = $return->user_id;
            $timestamp = $return->timestamp;
            if (verify_time($timestamp, 5)) {
                return view('frontend.welcome-owner', ['user_id' => $user_id]);
            }
        }
        return redirect(url('/'));
    }
    ...
    public function _approveUser(Request $request)
    {
        ...
        } elseif ($request == 'reqest_a_owner') {
            $update = $user_model->updateUserRole($user_id, 5);
        }
        if ($update) {
            $request_flag = false;
    ...
    public function _becomeAOwner(Request $request)
    {
        // if (get_option('enable_partner_registration', 'on') == 'off') {
        //     return $this->sendJson([
        //         'status' => 0,
        ...
        try {
            ...
            $role = $user_model->getRoleByName('owner');
            $user_model->updateUserRole($user->getUserId(), $role->id);
        ...
        if (!$user) {
            ...
            do_action('hh_user_registered_as_owner', $user->getUserId(), $password);
            $return = [
                'user_id' => $user->getUserId(),
                'timestamp' => time()
            ];
            return $this->sendJson([
                'status' => 0,
                'force_redirect' => url('welcome-owner', ['return' => base64_encode(json_encode($return))]),
            ]);
        }
    }
