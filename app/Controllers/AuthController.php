    ...
    class AuthController extends Controller
    {
        ...
        public function _postResetPassword(Request $request)
        {
            ...
            if (is_null($user)) {
                ...
                $admin_email = get_option('email_from_address');
                $admin_agency = get_option('email_from');
                if (!$user) {
                    return $this->sendJson([
                        'status' => 0,
                    ...
                    $sent = send_mail($admin_email, $admin_agency, $user->email, $subject, $content);
                    if (!$sent) {
                        return $this->sendJson([
                            'status' => 0,
        ...
        public function _getOwnerLogin()
        {
            return view('dashboard.owner-login', ['bodyClass' => 'authentication-bg authentication-bg-pattern']);
        }
        ...
        public function _postLogin(Request $request)
        {
            ...
            if (Sentinel::check()) {
                ...
                if(is_owner()) {
                    $_SESSION['hh_language'] = 'hr';
                }else {
                    $_SESSION['hh_language'] = 'en';
                }
                return $this->sendJson([
                    'status' => 1,
