...
class OptionController extends Controller
{
    ...
    private function _home_inventory()
    {
        ...
        if ($post_id && $start && $end) {
            ...
            if ($booking_type == 'per_night') {
                ...
                $list_booking = $avai_model->getAvailabilityItems($post_id, $start, $end);
            } else {
            ...
            if ($list_booking['total']) {
                foreach ($list_booking['results'] as $item) {
                    if ($booking_type == 'per_night') {
                        ...
                        $event = '<div class="booking-item booking-"><div class="author"></div></div>';
                    } else {
                        $params = [
                            'service_id' => $post_id,
                            'service_type' => 'home',
    ...
    public function _saveSetting(Request $request)
    {
        ...
        if ($fields) {
            ...
            $optionValue['primary_currency'] = "EURO";
            $optionValue = serialize($optionValue);
            if ($hasOption) {
    ...
    public function wspayUpdate(Request $request){
        
        foreach($request->input() as $kx => $vx){
            if($kx!="_token"){
