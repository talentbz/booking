...
class CheckoutController extends Controller
{
    ...
    public function _checkoutAction(Request $request)
    {
        ...
        if($paymentMethod=="wspay"){
            return $this->sendJson([
                    'status' => 1,
                    'message' => view('common.alert', ['type' => 'danger', 'message' => 'next_page'])->render(),
                    'redirect'=>route('checkoutWspay',['id'=>$new_booking_id])
                ]);
        }
    ...
    public function _thankyouPage(Request $request, $orderId=null)
    {
        //$orderID = request()->get('_orderID');
        $orderID = $orderId;
        /*
    ...
    public function checkoutWspay(){
        ...
        require 'app/Models/SignatureHelper.php';
        
        $shopId = Setting::main()->wspay_shopid;
        $secretKey = Setting::main()->wspay_secret_key;
        $orderId = $booking->ID;
        $amount = $booking->total;

        $helper = new \WSPayLib\SignatureHelper($secretKey);
        $calculatedSignature = $helper->calculateSignature($shopId, $orderId, $amount);
        $booking->wspay_signature = $calculatedSignature;
        $booking->save();
        
        $data['signature'] = $calculatedSignature;
        $data['setting'] = Setting::main();

         
         
        return view('frontend.checkoutWspay',$data);
    }
    
    
    public function wspayReturn(){
       
       return view('frontend.wspayReturn');
        
    }
    
    public function wspaySuccess(){
        
        \Cart::get_inst()->deleteCart();
        $Booking = Booking::find(request()->input('ShoppingCartID'));
        if(!isset($Booking->ID)){
            $Booking = new Booking;
        }
        ...
        // }
        $Booking->transaction_id = request()->input('WsPayOrderId');
        $Booking->payment_data = json_encode(request()->input());
        $Booking->save();
