...
class MailSystem
{
    public function __construct()
    {
        ...
        add_action('hh_user_registered_as_owner', [$this, '_sendOwnerRequest'], 10, 2);
    }
    ...
    public function _sendWelcomeOwner($user_id, $password)
    {
        $user = get_user_by_id($user_id);
        $user_email = $user->email;
        $subject = sprintf(__('[%s] - You have successfully created an account'), get_option('site_name'));
        $content = view('frontend.email.welcome-owner', ['user' => $user, 'password' => $password])->render();
        $from = get_option('email_from_address');
        $from_name = get_option('email_from');
        send_mail($from, $from_name, $user_email, $subject, $content);
    }
    ...
    public function _sendOwnerRequest($user_id, $password)
    {

        $this->_sendWelcomeOwner($user_id, $password);
