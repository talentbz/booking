...
class Home extends Model
{
    ...
    public function getSearchResult($data)
    {
        ...
        if (!empty($data['is_featured'])) {
            $sql->whereRaw("is_featured = 'on'");
        }

        $sql->whereRaw("status = 'publish'");
        if (!empty($data['price_filter'])) {
            $min_max = get_origin_filter_price($data['price_filter']);
            $whereStr = " (home.base_price >= {$min_max['min']} and home.base_price <= {$min_max['max']}) or (home_price.price_per_night >= {$min_max['min']} and home_price.price_per_night <= {$min_max['max']}) ";
            $sql->whereRaw($whereStr);
        }
    ...
    public function listOfHomes($data)
    {
        ...
        $sql = DB::table($this->getTable())->selectRaw("SQL_CALC_FOUND_ROWS home.*, media.media_url")->where('home.status', 'publish')->orderBy($data['orderby'], $data['order']);
        $sql->leftJoin('media', function ($join){
            $join->on('home.thumbnail_id', '=', 'media.media_id');
        });
        if (!empty($data['id'])) {
    ...
    public function getAllHomes($data)
    {
        ...
        if (!is_admin()) {
            if(is_owner()) {
                $sql->where('owner', get_current_user_id());
            }else {
    ...
    public function getSpecialOffering($data)
    {
        ...
        if($data['table'] = 'last_minute'){
            $table = 'view_last_minute';
        }

        $sql = DB::table($table)->selectRaw("SQL_CALC_FOUND_ROWS *");
