...
class HomeAvailability extends Model
{
    ...
    public function getHomeAvailabilityCustomBook($home_id)
    {
        $sql = DB::table($this->getTable())->where('home_id', $home_id)->where('booking_id', 0);
    ...
    public function _saveCustomAvailability($home_id, $start, $end)
    {
        $this->createAvailability([
            'home_id' => $home_id,
    ...
    public function deleteAvailabilityByID($availiabilityID)
    {
        return DB::table($this->table)->where('ID', $availiabilityID)->delete();
    }

    public function updateType($availiabilityID, $type)
    {
        $data = array('type' => $type);
        return DB::table($this->getTable())->where('ID', $availiabilityID)->update($data);
    }
