...
<div class="hh-search-bar-wrapper">
    <div class="hh-search-bar-buttons">
        ...
        $homeType = request()->get('home-type');
        ...
        ?>
        <div class="hh-button-item button-location form-group" style="padding: 17px;">
            <input type="text" id="demo5" name="address" class="form-control typeahead" style="color: #808080; font-size: 14px; font-weight: 400;" autocomplete="off" value="{{$address}}" placeholder="{{__('Enter a location ...')}}">
            ...
            <input type="hidden" id="home-type" name = "home-type" value="{{$homeType}}">
            <input type="hidden" id="is_featured" name="is_featured" value="{{$is_featured}}">
        </div>
        <?php
        $booking_type = request()->get('bookingType', 'per_night');
        ?>
        @if($booking_type == 'per_night')
            <div class="button-date date-wrapper date date-double">
                <?php
                ...
                <input type="text" class="input-hidden check-in-field" name="checkIn" value="{{ $post_checkIn }}">
                <input type="text" class="input-hidden check-out-field" name="checkOut" value="{{ $post_checkOut }}">
                <span class="check-in-render" style="color: #808080;"
                    data-date-format="DD.MM.YYYY.">{{ date_format(new DateTime($checkIn), 'd.m.Y.') }}</span>
                <span class="divider"></span>
                <span class="check-out-render" style="color: #808080;"
                    data-date-format="DD.MM.YYYY.">{{ date_format(new DateTime($checkOut), 'd.m.Y.') }}</span>
            </div>
        @endif
        ...
        <div class="dropdown dropdown-button dropdown-button-price">
            <div class="hh-button-item button-price form-group" data-toggle="dropdown" aria-haspopup="true"
                 role="article"
                 aria-expanded="false">
                <span class="text" style="font-size: 14px; color: #808080; font-weight: 400;">{{__('Price')}}</span>
                <div class="dropdown-menu">
        ...
        <div class="dropdown dropdown-button dropdown-button-more-filter" >
            <div class="hh-button-item button-more-filter form-group custom_advanced_search" data-toggle="dropdown" aria-haspopup="false"
                 role="article"
                 aria-expanded="true">
                <span class="text" style="font-size: 14px; color: #808080; font-weight: 400;"><?php echo __('More filters'); ?></span>
                <div class="dropdown-menu">
                    <?php
                    ...
                    @if (!empty($terms))
                        @foreach ($terms as $term_name => $term)
                            ...
                            <div class="item-filter-wrapper" data-type="{{ $term_name }}">
                                ...
                                <div class="label" style="font-weight: bolder; color: #115571; font-size: 19px;text-transform: uppercase;">@if($term['label'] == 'Home Amenity') AMENTIES @elseif($term['label'] == 'Home Type') TYPE @else {{ $term['label'] }} @endif</div>
                                    <?php
                        ...
                        @endforeach
                        <div class="item-filter-wrapper" id="special-offer">
                            <div class="label" style="font-weight: bolder; color: #115571; font-size: 19px;text-transform: uppercase;">Special Offer</div>
                            <div class="content">
                                <div class="row">
                                    <div class="col-lg-4 mb-1">
                                        <div class="item checkbox  checkbox-success ">
                        ...
                        <?php
                                ...
                                <div class="item-filter-wrapper" id="home-facilities">
                                    ...
                                    foreach ($facilities_list as $key => $value) { ?>
                                        <div class="label" style="font-weight: bolder; color: #115571; font-size: 19px;text-transform: uppercase;">{{$value['title']}}</div>
                                        <div class="content">
                                            <div class="row">
                    ...
                    @endif
                    <a href="javascript:void(0)"
                       class="apply-more-filter btn btn-primary btn-xs right" style="background:#D9BA7A;border-color: #D9BA7A;font-size: 19px;">{{__('Apply')}}</a>
                </div>
            </div>
        </div>
        <div class="hh-toggle-map-search" style="margin-left: 5px;border: 1px solid #e2e2e2; border-radius: 3px; padding: 6px 20px;">
            <label for="hh-map-toggle-search" style="color: #808080;margin-top: 8px;" class="mr-2">{{__('Show Map')}}</label>
            <input id="hh-map-toggle-search" type="checkbox" data-plugin="switchery" data-color="#1bb99a"
