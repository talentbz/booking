...
<div class="card-box mt-3 cart-information cart-home-item">
    ...
    <ul class="menu cart-list">
        ...
        <li>
            <span>{{ _n("0::Discount (%s %)][1::Discount (%s %)][2::Discount (%s %)]", $cart['discount_percent']) }}</span>
            <span>{{ convert_price(($basePrice - $cart['amount'])) }}</span>
        </li>
        @endif
        @if ($extraPrice > 0)
            <li>
    ...
    <ul class="menu cart-list">
        <li>
            <span>{{__('Advanced 40%')}}</span>
            <!-- <span>{{ convert_price($cart['amount']) }}</span> -->
            <span>{{ convert_price(($basePrice * 0.4)) }}</span>
        </li>
        <li>
            <span>{{__('Payment deadline')}}</span>
            <!-- <span>{{ convert_price($cart['amount']) }}</span> -->
            <span>{{ date('d.m.Y.', strtotime('+2 day', $checkIn)) }}</span>
        </li>
    </ul>
    <h5 class="title">{{__('  ')}}</h5>
    <ul class="menu cart-list">
        <li>
            <span>{{__('Remaining Amount 60%')}}</span>
            <!-- <span>{{ convert_price($cart['amount']) }}</span> -->
            <span>{{ convert_price(($basePrice * 0.6)) }}</span>
        </li>
        <li>
            <span>{{__('Payment deadline')}}</span>
            <!-- <span>{{ convert_price($cart['amount']) }}</span> -->
            <span>{{ date('d.m.Y.', $checkOut) }}</span>
        </li>
    </ul>
    @endif
</div>
