...
abstract class Gateways
{
    ...
    public function customSuccessUrl($status = 1)
    {
        $args = [
            '_payment' => 'paypal',
            '_status' => $status
        ];
        $args['_hash'] = $this->customCreateHash($args);
    ...
    public function customCancelUrl()
    {
        $args = [
            '_payment' => 'paypal',
            '_status' => 0,
    ...
    public function customCreateHash($data)
    {
        $hash_string = $data['_payment'] . '|' . $data['_status'];
        if (isset($data['_transactionID'])) {
            $hash_string .= '|' . $data['_transactionID'];
        }
        return hash_hmac('sha256', $hash_string, uniqid());
    }
