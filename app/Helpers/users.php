...
function user_can_edit_service($service_object, $user_id = null): bool
{
    ...
    if (is_admin($user_id) || ((int)$user_id === (int)$service_object->author) || ((int)$user_id === (int)$service_object->owner)) {
        return true;
    }
    return false;
}
...
function is_owner($user_id = '')
{
    ...
    if ($user_data) {
        return $user_data->inRole('owner') ? true : false;
    }
    return false;
}
...
function get_customer_user_list()
{
    $user_model = new \App\Models\User();
    $result = $user_model->getCustomersList();
    return $result;
}
