...
function agent_user_can_review($user_id, $author_id, $type = 'home')
{
    return \App\Controllers\CommentController::get_inst()->agentUserCanReview($user_id, $author_id, $type);
}
...
function get_menu_dashboard()
{
    ...
    } else if(Sentinel::inRole('owner')){
        $menu = Config::get('awebooking.owner_menu');
    } else {
...
{
    $countries = [];
    $countries_data = file_get_contents(public_path('vendor/countries/countries1.json'));
    ...
    if (!empty($countries_data) && is_array($countries_data)) {
        foreach ($countries_data as $country) {
            $countries[$country['id']] = [
                ...
                'styles' => 'style="background-image:url(' . asset('vendor/countries/flag/24x24/' . $country['alpha2'] . '.png') . ');"',
...
function list_countries1($key = null)
{
    $countries = [];
...
function get_comment_agent_number($agent_id, $type = 'home')
{
    $comment = new CommentAgent();
    $comment_count = $comment->getCommentCountByID($agent_id, $type);
    return $comment_count;
}
...
function limitString($str, $limit, $mark)
{
    return mb_strimwidth($str, 0, $limit, $mark);
}

function getQrCode($service_id, $service_type){
    ...
    $service_url = utf8_encode($service_url);
    echo '<div class="qrcode-render">';
    echo QrCode::size(100)->generate($service_url);
    echo '</div>';
}
