...
class Paypal extends Gateways
{
    ...
    public function sendTransaction($tip_price)
    {
        // TODO: Implement purchase() method.
        $this->setDefaultParams();
        $payer = new Payer();
        ...
        $item = new Item();
        $item->setName("Review and tip")
            ->setPrice($tip_price);
        $itemList = new ItemList();
        $itemList->setItems([$item]);
        $amount = new Amount();
        $amount->setCurrency('EURO')->setTotal($this->convertPrice($tip_price, get_currency_by_unit('EURO')));

        $transaction = new Transaction();
        $transaction->setAmount($amount)->setItemList($itemList)->setDescription("Review and tip")->setInvoiceNumber(uniqid());

        $redirectUrls = new RedirectUrls();
        $redirectUrls->setReturnUrl($this->customSuccessUrl())->setCancelUrl($this->customCancelUrl());
